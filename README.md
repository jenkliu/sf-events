...
| **Gather SF (page)** | <https://www.gathersf.org/events> | Framer page; reads Event JSON-LD on the page and follows its Luma / Partiful / Eventbrite links. No key needed. |
| **Gather SF (calendar)** | Google Calendar API | The same nonprofit's public calendar. Overlaps the page (deduped). |
...
Without any setup, this pulls **The Faight**, **Madrone**, and **Gather SF's events page**
(no key needed) and writes `events.json`.

To also include the three Google Calendars, set a Google API key:
...
- **Hosts its events on Luma / Partiful / Eventbrite?** Those pages carry schema.org `Event`
  JSON-LD, so the Gather SF route works for them too: point `GATHER.page` at the listing page
  (or copy `fetchGatherSF()`) — it reads JSON-LD on the page, then follows the ticket links and
  reads the JSON-LD (or `__NEXT_DATA__`) there.
- **Has a headless CMS or its own API?** Add a `fetchX()` that returns the normalized shape.

## Tests

The parsers (JSON-LD extraction, ticket-link harvesting, `__NEXT_DATA__` fallback, and the
Gather SF source end to end against a local fixture server) have offline tests — no network:
...
2. (Optional) Add a `GOOGLE_API_KEY` repository secret (**Settings → Secrets and variables →
   Actions**) to also pull the Google Calendars (Wave Collective, Lower Haight Local, Gather
   SF's calendar). Without it, the deployed site still gets The Faight + Madrone + Gather SF's
   events page.
