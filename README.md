...
| **The Commons** | Luma JSON feed | Community/coworking space on Laguna St, Hayes Valley. Calendar `cal-ahTi4ptrN9WCYkg`. Workshops, coaching, socials and the occasional retreat; falls back to Community & Social. `get-items` returns no descriptions like tiat, but at ~85 upcoming events fetching all of them per-event isn't worth it — only events in the next 2 weeks (~32) get a real description fetched and re-categorized against it. |
...
- **Events page renders nothing in the HTML?** The listing is probably a third-party widget
  (look for an embed like `elfsight-app-<id>`), and the widget's own config usually names the
  feed behind it — that's how Civic Joy Fund's calendar was found. See `docs/event-sources.md`.
