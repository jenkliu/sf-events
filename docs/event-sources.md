...
- **tiat (the intersection of art & technology)** — 151 Powell St, downtown SF (Union Square). Page: https://www.tiat.place/events — A 501(c)(3) art/tech gallery and experimental hub (opened Nov 2025 via the city's Vacant to Vibrant program). Hosts new media exhibitions, maker sessions, workshops (projection mapping, computational poetry, generative media), salons, and occasional raves — roughly Arts & Performance, plus some Nightlife/Music and Community (workshops, book clubs). Events are actually hosted and listed on **Luma**: https://luma.com/tiat (vanity slug `tiat`). Luma calendars can usually be pulled via an unofficial public JSON endpoint keyed on an opaque `calendar_api_id` (not the vanity slug) embedded in the calendar page's data — this needs to be extracted by loading https://luma.com/tiat and inspecting the page's embedded JSON/network requests. **Not yet wired up**: both `tiat.place` and `luma.com` are unreachable from this sandboxed dev environment's network egress policy, so the exact `calendar_api_id` and JSON response shape haven't been confirmed here. Not on DoTheBay/Do415. To finish: fetch https://luma.com/tiat from an unrestricted environment, grab the `calendar_api_id`, confirm the `calendar/get-items`-style response shape, then add a `fetchLuma()` source to `scrape.mjs` following the pattern of `fetchDoTheBay()`.
